...
QUADRA_ASCII_FORMAT = {  # Quadra name: [Position, Length, required, lambda expr to get value from move line, lambda expr to get value from journal]
    ...
    "Date échéance": [64, 6, False, lambda l: min(l.mapped('date_maturity')).strftime("%d%m%y")],
...
class ExportASCII(models.TransientModel):
    ...
    def generate_file(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': self.env.ref('quadra_export_ascii.account_export_ascii_result_form_view').id,
            'res_id': self.id,
            'target': 'new',
        }
